...
def tfce(observations, ch_adjacency_sparse, n_permutations=512, alpha=0.05):
    ...
    clustering algorithms without internal logic modifications.

    Parameters
    ----------
    observations : ndarray, shape (n_subjects, n_times, n_channels)
        The data observations for each subject, time point, and channel.
    ch_adjacency_sparse : scipy.sparse.csr_matrix
        The channel adjacency matrix.
    n_permutations : int, optional
        Number of permutations to calculate, by default 512.
    alpha : float, optional
        Significance threshold, by default 0.05.

    Returns
    -------
    clusters_mask : ndarray of bool
